/**
 * Run this code with valgrind ./namecompare --leak-check=full
 */
...
class NameCompareTest : public CppUnit::TestCase {

public:
  NameCompareTest(std::string name) : CppUnit::TestCase(name) {
  ...
  void print_namecompare(std::string s1, std::string s2, int prev, int cur, int value) {
  ...
  void traverse_cursors(std::string s1, std::string s2) {
...
test_name_comparison() {
  NameCompareTest * nct = new NameCompareTest("Name comparison unit tests");
  nct->runTests();
  delete nct;
}
