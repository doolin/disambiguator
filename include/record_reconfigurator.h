 ...
 *
 * Private:
 *         const map < const Record *, RecordList, cSort_by_attrib > * reference_pointer: a pointer to a patent tree, which can be obtained in a cBlocking_Operation_By_Coauthor object.
 *         const unsigned int coauthor_index: the index of the coauthor column in many columns.
 *
 * Public:
 *         Reconfigurator_Coauthor ( const map < const Record *, RecordList, cSort_by_attrib > & patent_authors):
...
private:
    const map < const Record *, RecordList, cSort_by_attrib > * reference_pointer;
